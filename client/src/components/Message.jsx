    ...
    <div className={`flex ${isUser ? "justify-end" : "justify-start"}`}>
      <div
        className={`max-w-[min(88%,820px)] text-[var(--app-text)] ${
          isUser
            ? "rounded-2xl border border-[var(--app-border)] bg-[var(--user-message-bg)] px-4 py-3 shadow-[var(--surface-shadow)]"
      ...
      >
        {message.isImage ? (
          <img
            src={message.content}
            className="max-h-[26rem] w-full rounded-2xl border border-[var(--app-border)] object-cover shadow-[var(--surface-shadow)]"
